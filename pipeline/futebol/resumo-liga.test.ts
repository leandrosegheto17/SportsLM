...
const inc: InconsistenciaPartida[] = [
  { tipo: 'clube-nao-mapeado', competicaoId: 'x', idProvedor: 1, nomeProvedorDiagnostico: 'A', contexto: 'c' },
  { tipo: 'clube-nao-mapeado', competicaoId: 'x', idProvedor: 2, nomeProvedorDiagnostico: 'B', contexto: 'c' },
  { tipo: 'partida-status-desconhecido', competicaoId: 'x', idPartidaProvedor: 1, statusBrutoDiagnostico: 'ZZ', contexto: 'c' },
  { tipo: 'clube-serie-a-sem-id', competicaoId: 'x', idProvedor: '9', nomeProvedorDiagnostico: 'C', contexto: 'c' },
  { tipo: 'partida-invalida', competicaoId: 'x', idPartidaProvedor: '3', motivo: 'm', contexto: 'c' },
];

describe('montarResumoLiga', () => {
  it('sem descartes: zera todos os motivos', () => {
    const r = montarResumoLiga({ partidas: 5, requisicoes: 2, foraDoRecorte: 0, inconsistencias: [] });
    expect(r).toEqual({
      partidas: 5,
      requisicoes: 2,
      descartes: {
  ...
  it('conta cada motivo e fora-do-recorte', () => {
    const r = montarResumoLiga({ partidas: 3, requisicoes: 1, foraDoRecorte: 4, inconsistencias: inc });
  ...
  it('partidas + descartes de partida = eventos recebidos', () => {
    const r = montarResumoLiga({ partidas: 3, requisicoes: 1, foraDoRecorte: 4, inconsistencias: inc });
    const d = r.descartes;
    const total = r.partidas + d['fora-do-recorte'] + d['clube-serie-a-sem-id'] + d['status-desconhecido'] + d['nome-externo-invalido'];
    expect(total).toBe(3 + 4 + 1 + 1 + 1);
  });
});

describe('linhaLogLiga', () => {
  const resumo = montarResumoLiga({ partidas: 3, requisicoes: 1, foraDoRecorte: 4, inconsistencias: inc });
  const args = { horario: new Date('2026-09-18T10:00:00Z'), liga: 'paulista', resultado: 'ok', resumo };
