...
const ID = { copa: '4725', lib: '4501', sul: '4724', paulista: '5767', carioca: '5688', mineiro: '5763', gaucho: '5691' };
...
function eventoSintetico(over: Record<string, unknown>): Record<string, unknown> {
  const base = (JSON.parse(fx('spk06-libertadores-past.json')) as { events: Record<string, unknown>[] })
    .events[0];
  return { ...base, ...over };
}
...
const SUL_PASSADO = corpo([
  eventoSintetico({
    idEvent: '9100001', idLeague: ID.sul, strLeague: 'Copa Sudamericana',
    strEvent: 'Botafogo vs Racing Club', strHomeTeam: 'Botafogo', strAwayTeam: 'Racing Club',
    idHomeTeam: idTsdb('botafogo'), idAwayTeam: '139999',
    strTimestamp: '2026-09-11T00:30:00', dateEvent: '2026-09-11', strTime: '00:30:00',
    intHomeScore: '2', intAwayScore: '0', strStatus: 'FT',
  }),
]);
const SUL_FUTURO = corpo([
  eventoSintetico({
    idEvent: '9100002', idLeague: ID.sul, strLeague: 'Copa Sudamericana',
    strEvent: 'Racing Club vs Botafogo', strHomeTeam: 'Racing Club', strAwayTeam: 'Botafogo',
    idHomeTeam: '139999', idAwayTeam: idTsdb('botafogo'),
    strTimestamp: '2026-09-19T23:00:00', dateEvent: '2026-09-19', strTime: '23:00:00',
...
const LIB_BOTAFOGO_FUTURO = eventoSintetico({
  idEvent: '9100003', strEvent: 'Botafogo vs Estudiantes de La Plata',
  strHomeTeam: 'Botafogo', strAwayTeam: 'Estudiantes de La Plata',
  idHomeTeam: idTsdb('botafogo'), idAwayTeam: '138000',
  strTimestamp: '2026-10-20T00:30:00', dateEvent: '2026-10-20', strTime: '00:30:00',
  intHomeScore: null, intAwayScore: null, strStatus: 'NS',
});
...
function respostasPadrao(): Record<string, string> {
  const libNext = (JSON.parse(fx('spk06-libertadores-next.json')) as { events: unknown[] }).events;
  return {
...
function criarAmbiente(agora: Date, cenario: Cenario = {}): Ambiente {
  ...
  const buscar: BuscadorHttp = (url) => {
    ...
    let chave = '';
    if (endpoint === 'eventspastleague.php') chave = `past:${u.searchParams.get('id') ?? ''}`;
    else if (endpoint === 'eventsnextleague.php') chave = `next:${u.searchParams.get('id') ?? ''}`;
    else if (endpoint === 'lookuptable.php') chave = `table:${u.searchParams.get('l') ?? ''}`;
    let texto: string | number | undefined;
    if (endpoint === 'eventsday.php') {
      texto = dias[`${u.searchParams.get('l') ?? ''}:${u.searchParams.get('d') ?? ''}`] ?? corpo(null);
    } else {
      texto = respostas[chave] ?? (endpoint === 'lookuptable.php' ? '' : corpo(null));
    }
    if (typeof texto === 'number') {
      return Promise.resolve({
        ok: false, status: texto,
  ...
  const espacador = criarEspacador({
    maxPorJanela: 28, janelaMs: 60000, intervaloMinMs: 2200,
    agora: () => relogio,
    dormir: (ms) => { relogio += ms; return Promise.resolve(); },
  });
  const adaptadorTsdb = criarAdaptadorTheSportsDB({ clubes: CLUBES, buscar, espacador, agora: () => agora });
  const registroTsdb = registrarProvedor<RefCompeticaoTheSportsDB>(adaptadorTsdb, (c) => {
    ...
    const pol = c.formato === 'mata-mata' ? 'nunca' : c.formato === 'misto' ? 'tentar' : 'sempre';
    const completa = (c.formato === 'grupos' || c.formato === 'pontos-corridos') && c.clubes.length > 0;
    return {
      competicaoId: c.id, idLigaProvedor: ref.id, idLiga: Number(ref.id), politicaTabela: pol,
      ...(ref.temporada !== undefined ? { temporadaProvedor: ref.temporada } : {}),
      ...(completa ? { clubesEsperados: c.clubes.length } : {}),
    };
  });
  return { urls, instantes, provedores: { 'football-data-org': registroBrasileirao(), thesportsdb: registroTsdb } };
}
...
function registroBrasileirao(): ProvedorRegistrado {
  ...
  const mk = (n: number, fin: boolean, [m, v]: [string, string], i: number): Partida => ({
    id: `bsa-${String(n)}-${String(i)}`, competicaoId: 'brasileirao-serie-a', rodada: n, fase: null,
    mandanteId: m, visitanteId: v,
    dataHora: fin ? '2026-09-10T19:00:00-03:00' : '2026-09-19T19:00:00-03:00',
    horarioDefinido: true, estadio: null,
  ...
  const linhas: LinhaClassificacao[] = clubes.map((clubeId, i) => ({
    competicaoId: 'brasileirao-serie-a', grupo: null, posicao: i + 1, clubeId,
    pontos: 1, jogos: 1, v: 0, e: 1, d: 0, gp: 1, gc: 1, sg: 0,
    aproveitamento: 100 / 3, ultimosCinco: ['E'],
  }));
  return registrarProvedor<{ competicaoId: string }>(
    {
      id: 'football-data-mock', orcamento: { porMinuto: 10 },
...
const rodar = (
  ...
  executarFluxoFutebol({ campeonatos, provedores: amb.provedores, estadoAnterior, agora });
...
function publicar(res: ResultadoFluxoFutebol, agora: Date): SnapshotsPublicados {
  return construirSnapshots({
    ...
    campeonatosConfig: CAMPEONATOS, clubesBase: CLUBES as ClubeBase[],
    esportes: carregarEsportesConfig(), zonas: null,
  });
}

const st = (r: ResultadoFluxoFutebol, id: string): string | undefined => r.status.futebol[id]?.resultado;
...
describe('COB-31 — ciclo 1 (2026-09-18, fixtures SPK-06/SPK-08)', () => {
  it('coleta ponta a ponta, isolamento, ordem RN-22, cota, snapshots e meta I-28', async () => {
    ...
    for (const id of ['paulista', 'carioca', 'mineiro', 'gaucho', 'supercopa-do-brasil']) {
    ...
    for (const id of ['brasileirao-serie-a', 'copa-do-brasil', 'libertadores', 'sul-americana']) {
      expect(st(res, id), `${id}: ${JSON.stringify(res.status.futebol[id])}`).toBe('atualizada');
    }
    ...
    const botLib = res.novoEstado.competicoes['libertadores']?.participacoes.find((p) => p.clubeId === 'botafogo');
    const botSul = res.novoEstado.competicoes['sul-americana']?.participacoes.find((p) => p.clubeId === 'botafogo');
    ...
    expect(sulPartidas.every((p) => p.externo !== undefined || p.mandanteId.startsWith('externo-') || p.visitanteId.startsWith('externo-'))).toBe(true);
    ...
    expect(bot.map((c) => c.competicao.id)).toEqual(
      expect.arrayContaining(['libertadores', 'sul-americana', 'copa-do-brasil', 'brasileirao-serie-a']),
    );
    ...
    for (const camps of Object.values(snap.futebolPorClube)) {
      for (const camp of camps) {
        for (const pt of camp.partidas) {
          if (pt.mandanteId.startsWith('externo-') || pt.visitanteId.startsWith('externo-')) {
...
describe('COB-31 — estaduais dentro da janela (2026-03-09)', () => {
  it('tabela parcial (Paulista), PEN e ausencia de fase (Carioca), sem-dados-provedor e corpo vazio', async () => {
    ...
    const res = await rodar(CICLO_JANELA, amb, estadoFutebolVazio(), CAMPEONATOS_COM_BRAGANTINO_NO_PAULISTA);
    ...
    expect(st(res, 'paulista'), JSON.stringify(res.status.futebol['paulista'])).toBe('atualizada');
    ...
    // Carioca (SPK-06): lookuptable com corpo vazio (HTTP 200) + jogo PEN. LACUNA COB-31 (G2):
    // liga de grupos sem nenhuma linha de tabela nao e "parcial" (linhas = 0), entao a
    // verificacao (numero-de-clubes-incorreto) descarta o lote inteiro, jogos inclusive.
    // Comportamento atual documentado aqui; nunca lanca, nunca carimba ultimaAtualizacao.
    expect(st(res, 'carioca')).toBe('inconsistente');
    expect(res.status.futebol['carioca']?.motivosInconsistencia).toEqual(['numero-de-clubes-incorreto']);
    expect(res.status.futebol['carioca']?.ultimaAtualizacao).toBeNull();
    ...
    for (const id of ['mineiro', 'gaucho']) {
      ...
      expect(res.novoEstado.competicoes[id]?.participacoes.every((p) => p.status === 'sem-dados' || p.status === 'nao-iniciado')).toBe(true);
    }
...
describe('COB-31 — PEN e ausencia de fase (Copa do Brasil, rodada 1, SPK-08)', () => {
  it('PEN vira finalizada com placar, sem fase e sem eliminar ninguem', async () => {
    ...
    const rodada1 = criarAmbiente(CICLO_JANELA, {
      rotas: { [`past:${ID.copa}`]: fx('spk08-copa-do-brasil-rodada1.json'), [`next:${ID.copa}`]: corpo(null) },
    });
    ...
    const pen = eventoSintetico({
      idEvent: '9100010', idLeague: ID.copa, strEvent: 'Palmeiras vs Externo FC',
      strHomeTeam: 'Palmeiras', strAwayTeam: 'Externo FC', idHomeTeam: idTsdb('palmeiras'), idAwayTeam: '139888',
      strTimestamp: '2026-03-04T23:00:00', dateEvent: '2026-03-04', strTime: '23:00:00',
      intHomeScore: '2', intAwayScore: '2', strStatus: 'PEN', strGroup: null, intRound: '0',
    });
    const amb = criarAmbiente(CICLO_JANELA, {
    ...
    expect(st(res, 'copa-do-brasil'), JSON.stringify(res.status.futebol['copa-do-brasil'])).toBe('atualizada');
...
describe('COB-31 — acumulo entre ciclos, 429 e isolamento', () => {
  it('finalizada nunca regride e partida que some e mantida (ADR-022)', async () => {
    ...
    const evt = (JSON.parse(fx('spk06-libertadores-past.json')) as { events: Record<string, unknown>[] }).events[0] as Record<string, unknown>;
    ...
    const amb2 = criarAmbiente(CICLO1, {
      ...
      dias: { [`${ID.lib}:2026-09-17`]: corpo(null), [`${ID.lib}:2026-09-18`]: corpo(null) },
    });
    ...
    for (const p of antes) expect(depois.some((d) => d.id === p.id), p.id).toBe(true);
  });
  ...
  it('429 no meio: restantes pausado-por-cota, dado anterior mantido, outro provedor segue, RN-22 na ordem', async () => {
    ...
    expect(r2.novoEstado.competicoes['libertadores']).toEqual(r1.novoEstado.competicoes['libertadores']);
    expect(r2.novoEstado.competicoes['copa-do-brasil']).toEqual(r1.novoEstado.competicoes['copa-do-brasil']);
  });

  it('isolamento: uma liga que lanca vira falha so dela; externo e demais ligas mantidos', async () => {
    const r1 = await rodar(CICLO1, criarAmbiente(CICLO1));
    const amb = criarAmbiente(CICLO1, { rotas: { [`past:${ID.sul}`]: '{ json quebrado' } });
    const r2 = await rodar(CICLO1, amb, r1.novoEstado);
    expect(st(r2, 'sul-americana')).toBe('falha');
    expect(r2.status.futebol['sul-americana']?.mensagemErro ?? '').not.toContain('json quebrado'.repeat(3));
    expect(st(r2, 'libertadores')).toBe('atualizada');
    expect(st(r2, 'copa-do-brasil')).toBe('atualizada');
    expect(r2.novoEstado.competicoes['sul-americana']).toEqual(r1.novoEstado.competicoes['sul-americana']);
  });
});
...
describe('COB-31 — eliminacao so com evidencia e cobertura I-28', () => {
  ...
  it('mede a cobertura (pares clube-competicao com dado real nas fixtures)', async () => {
    ...
    const r1 = await rodar(CICLO1, criarAmbiente(CICLO1));
    const r2 = await rodar(CICLO_JANELA, criarAmbiente(CICLO_JANELA), r1.novoEstado, CAMPEONATOS_COM_BRAGANTINO_NO_PAULISTA);
    ...
    for (const c of CAMPEONATOS_COM_BRAGANTINO_NO_PAULISTA) {
      ...
      const ligaAtualizada = (finais[c.id]?.competicao.ultimaAtualizacao ?? null) !== null;
      total += c.clubes.length;
      comDado += ok;
      if (ligaAtualizada) emLigaComLote += c.clubes.length;
      porLiga.push(`${c.id}: ${String(ok)}/${String(c.clubes.length)}${ligaAtualizada ? '' : ' (sem lote)'}`);
    }
